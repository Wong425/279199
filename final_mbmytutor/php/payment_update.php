...
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        $sqlinsertpayment = "INSERT INTO `tbl_orders`(`receipt_id`, `order_paid`,`order_status`) VALUES ('$receiptid','$amount','$status')";
        ...
        if ($conn->query($sqlinsertpayment) && $conn->query($sqpupdatecart)){
            ...
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    $crid = $rows['course_id'];
                    $crqty = $rows['cart_qty'];
                    $updateproductsqty = "UPDATE tbl_subjects set subject_qty = (subject_qty-$crqty) WHERE subject_id = '$crid'";
